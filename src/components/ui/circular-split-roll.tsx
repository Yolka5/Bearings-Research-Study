...
const defaultItems: CircularSplitRollItem[] = [
  { id: 0, title: "סוגי מיסבים", image: "/Bearings1.jpeg", alt: "סוגי מיסבים" },
  { id: 1, title: "איך בוחרים מיסב", image: "/Bearings2.jpg", alt: "איך בוחרים מיסב" },
  { id: 2, title: "קיבוע גוף המיסב", image: "/Bearings3.jpg", alt: "קיבוע גוף המיסב" },
  { id: 3, title: "חומרי ייצור", image: "/Bearings4.jpg", alt: "חומרי ייצור" },
];
...
function getStrength(value: number) {
  return gsap.utils.clamp(0, 1, gsap.utils.mapRange(-1, 1, 0, 1, value));
}
          ...
          >
            <div className="relative h-[78vh]">
              {safeItems.map((item) => (
                <div
                  key={item.id}
                  className="circular-scroll-showcase__left-item pointer-events-none absolute left-1/2 top-1/2 w-full origin-center whitespace-nowrap text-center font-medium leading-none tracking-[-0.04em] opacity-0 will-change-[transform,opacity]"
                  style={{ fontSize: "var(--css-title-size)" }}
                >
                  {item.title}
                </div>
              ))}
